...
from src.libs.messages import print_boot_message, print_success_message, print_error_message
...
def openConversation(
    ...
    try:
        ...
        if 0 <= idx < len(conversation_dirs):
            ...
            rag_chain, vectorstore, text_splitter, llama_embeddings, llm_instance = ragSystem(current_memory_path, chroma_db_dir_path, is_new_session=False)

            current_conversation_history = loadConversation(current_memory_path, conversation_filename)
            
            print_success_message(f"Successfully loaded conversation from '{conversation_hash_name}'.")
