    ...
    try:
        if path.is_file():
            ...
            if path.suffix.lower() == ".md":
                ...
                print_info_message(f"Detected .txt file. Loading as plain text.")
                loader = TextLoader(str(path))
                ingested_documents.extend(loader.load())
            elif path.suffix.lower() == ".json":
                print_info_message(f"Detected .json file. Loading with custom JSON plaintext loader.")
                loader = JsonPlaintextLoader(str(path))
                ingested_documents.extend(loader.load())
            else:
                print_info_message(f"Attempting to load with UnstructuredFileLoader for unknown type.")
                loader = UnstructuredFileLoader(str(path))
                ingested_documents.extend(loader.load())

        elif path.is_dir():
            print_info_message(f"Ingesting documents from directory: '{path_to_ingest}'")
            ...
            for json_file in json_files:
                print_info_message(f"Found JSON file: '{json_file}'. Loading with custom JSON plaintext loader.")
                json_loader = JsonPlaintextLoader(str(json_file))
                ingested_documents.extend(json_loader.load())

            if not ingested_documents:
                print_note_message(f"No .md, .txt, or .json files found in '{path_to_ingest}'.")
        else:
            print_error_message(f" Invalid path type: '{path_to_ingest}'. Please provide a file or a directory.")
            return

        if not ingested_documents:
            print_note_message(f"No documents found to ingest at '{path_to_ingest}'.")
            return
        ...
        print_info_message(f" Adding new chunks to vector store (safe mode: 1 by 1)...")
        success, failed = 0, 0
        ...
        for i, chunk in enumerate(new_chunks, start=1):
            try:
                ...
                if i % 20 == 0 or i == len(new_chunks):
                    print_success_message(f"Added {success}/{i} chunks so far.")
            except Exception as e:
                failed += 1
                print_error_message(f" Failed on chunk {i}: {e}")

        print_info_message(f"Ingestion finished. "
        ...
        print_error_message(f"An error occurred during ingestion from '{path_to_ingest}': {e}")

def ingestConversationHistory(conversation_history: list, vectorstore, text_splitter, embeddings):
    ...
    if new_chunks:
        try:
            ...
            print_success_message(f"Successfully added {len(new_chunks)} conversation chunks to the vector store.")
        except Exception as e:
            print_error_message(f"Failed to add conversation chunks to vector store: {e}")
    else:
        print_note_message(f"No new chunks were created from the conversation history.")